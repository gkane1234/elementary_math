# Notes — `u_sub_preset_exp_chain` (`calc_indef_int_logarithmic_rule_and_exponentials_with_substitution`)

Showcase gallery for a **named u-sub knob**, not a separate catalog leaf.
Forces `u_sub_form_preset=exp_chain` and `u_sub_construction=catalog` via
`extra_settings` in `gen_examples.py` SECTIONS.

- **Skill:** exp of poly/trig with matching chain factor
