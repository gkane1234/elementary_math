# Notes — `pc_probability_independent_dependent_events_word_problems` (`Probability: Independent/dependent events, word problems`)
...
- **Skill:** Student solve the problem for probability: independent/dependent events, word problems.
- **D=0:** compound P with story; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{{A fair coin is flipped and a fair die is rolled. What is the probability of getting heads and rolling an even number?}}$` | `$\frac{1}{4}$` | evaluate/rewrite |
| 8 | 101 | `$\text{{A fair coin is flipped and a fair die is rolled. What is the probability of getting heads and rolling an even number?}}$` | `$\frac{1}{4}$` | evaluate/rewrite |
| 16 | 101 | `$\text{{A fair coin is flipped and a fair die is rolled. What is the probability of getting heads and rolling an even number?}}$` | `$\frac{1}{4}$` | evaluate/rewrite |
| 22 | 101 | `$\text{{A fair coin is flipped and a fair die is rolled. What is the probability of getting heads and rolling an even number?}}$` | `$\frac{1}{4}$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §11.2 | https://openstax.org/books/precalculus-2e/pages/11-2-introducing-probability | compound P with story |
...
- Single story frame at sampled D — add OpenStax application frames.
...
- **Variety:** Old-path table shows one prompt shape across D bands sampled (seed=101) — `$\text{{A fair coin is flipped and a fair die is rolled. What is the p...`.
