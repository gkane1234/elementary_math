# Notes — `pc_polynomial_graphs_real_zeros_and_end_behavior` (`Graphs, real zeros, and end behavior of polynomial functions`)
...
- **Skill:** Student analyze the polynomial for graphs, real zeros, and end behavior of polynomial functions.
- **D=0:** zeros, end behavior, turning points; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Describe the end behavior of } f(x)=-3x^{3} - 2x^{2} - 3.$` | `$x\to-\infty:f(x)\to\infty;\quad x\to\infty:f(x)\to-\infty$` | evaluate/rewrite |
| 8 | 101 | `$\text{Describe the end behavior of } f(x)=5x^{4} - x^{2} - 3.$` | `$\text{as }x\to\pm\infty,\ f(x)\to\infty$` | evaluate/rewrite |
| 16 | 101 | `$\text{Describe the end behavior of } f(x)=7x^{5} + 2x^{2} - 1.$` | `$x\to-\infty:f(x)\to-\infty;\quad x\to\infty:f(x)\to\infty$` | evaluate/rewrite |
| 22 | 101 | `$\text{Describe the end behavior of } f(x)=6x^{7} - 4x^{2} - 8.$` | `$x\to-\infty:f(x)\to-\infty;\quad x\to\infty:f(x)\to\infty$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §3.4 | https://openstax.org/books/precalculus-2e/pages/3-4-graphs-of-polynomial-functions | zeros, end behavior, turning points |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/3-4-graphs-of-polynomial-functions.md`
...
- **Reuse:** Reuse poly skeleton / end-behavior analyzer.
