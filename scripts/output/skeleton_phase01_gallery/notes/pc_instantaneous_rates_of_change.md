# Notes — `pc_instantaneous_rates_of_change` (`Instantaneous rates of change`)
...
- **Skill:** Student find the instantaneous rate of change for instantaneous rates of change.
- **D=0:** derivative at point from f; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Find }f'(4)\text{ for }f(x)=x^{2}.$` | `$8$` | evaluate/rewrite |
| 8 | 101 | `$\text{Find }f'(2)\text{ for }f(x)=x^{2}.$` | `$4$` | evaluate/rewrite |
| 16 | 101 | `$\text{Find }f'(3)\text{ for }f(x)=x^{2}.$` | `$6$` | evaluate/rewrite |
| 22 | 101 | `$\text{Find }f'(3)\text{ for }f(x)=x^{2}.$` | `$6$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §12.3 | https://openstax.org/books/precalculus-2e/pages/12-3-derivatives | derivative at point from f |
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_instantaneous_rates_of_change`.
