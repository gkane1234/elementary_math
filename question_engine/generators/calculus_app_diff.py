...
def _absolute_extrema(topic: str, settings: dict) -> list[Question]:
    """Closed-interval EVT; stamps live-loop form_id / generator."""
    from question_engine.frameworks.primitives.calc_app_diff import (
        ABSOLUTE_EXTREMA_GENERATOR,
        sample_app_diff,
    )
    ...
    def build() -> tuple[str, str, str | None]:
        item = sample_app_diff("absolute_extrema", settings, rng=random)
        fid = item.form_id
        snap = item.metadata.get("spec_snapshot")
        build._last_meta = {  # type: ignore[attr-defined]
            **item.metadata,
...
def _motion_along_a_line(topic: str, settings: dict) -> list[Question]:
    ...
    def build() -> tuple[str, str, str | None]:
        item = sample_app_diff("motion", settings, rng=random)
        fid = item.form_id
        snap = item.metadata.get("spec_snapshot")
        build._last_meta = {  # type: ignore[attr-defined]
            **item.metadata,
...
def _motion_along_a_line_integral(topic: str, settings: dict) -> list[Question]:
    """Displacement from v(t); stamps live-loop form_id / generator."""
    from question_engine.frameworks.primitives.calc_app_diff import (
        MOTION_INTEGRAL_GENERATOR,
        sample_app_diff,
    )
    ...
    def build() -> tuple[str, str, str | None]:
        item = sample_app_diff("motion_integral", settings, rng=random)
        fid = item.form_id
        snap = item.metadata.get("spec_snapshot")
        build._last_meta = {  # type: ignore[attr-defined]
            **item.metadata,
...
def _differentials(topic: str, settings: dict) -> list[Question]:
    """dy = f'(x) dx; leftover lockout; stamps live-loop form_id / generator."""
    from question_engine.frameworks.primitives.calc_app_diff import (
    ...
    def build() -> tuple[str, str, str | None]:
        item = sample_app_diff("differentials", settings, rng=random)
        fid = item.form_id
        snap = item.metadata.get("spec_snapshot")
        build._last_meta = {  # type: ignore[attr-defined]
            **item.metadata,
