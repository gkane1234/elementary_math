...
- **D=8 seed=101:** $4\text{ less than }4\text{ times a number is }32\text{. Find the number.}$ → $9$ — frame_id=number_two_step, steps=two
- **D=8 seed=207:** $\text{Casey buys kites at \$3 each and a \$2 cover. The total is \$14. How many kites did Casey buy?}$ → $4\text{ kites}$ — frame_id=money_two_step, steps=two
- **D=16 seed=101:** $\text{Julian buys watches at \$7 each and a \$2 cover. The total is \$65. How many watches did Julian buy?}$ → $9\text{ watches}$ — frame_id=money_two_step, steps=two
- **D=16 seed=207:** $\text{The number of textbooks Casey bought was 3 more than twice the number of kites. Casey bought 11 textbooks. How many kites were there?}$ → $4\text{ kites}$ — frame_id=compare_twice, steps=two
...
**UNCLEAR** — old path dumps “applies two operations, giving $2x+2=2$”. Default rotates IA §2.2 `number_two_step`, `compare_twice` (k more than twice), `earnings_two_step`, and `money_two_step` (unit+fee) across seeds and D.

## Proposed engine (reuse vs new) — proposal only

- **Reuse:** SolveLinear two-step + `wp_packaging` (wired; frames rotate).
