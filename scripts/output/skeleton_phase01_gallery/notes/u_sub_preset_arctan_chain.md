# Notes — `u_sub_preset_arctan_chain` (`calc_indef_int_inverse_trigonometric_with_substitution`)

Showcase gallery for a **named u-sub knob**, not a separate catalog leaf.
Forces `u_sub_form_preset=arctan_chain` and `u_sub_construction=catalog` via
`extra_settings` in `gen_examples.py` SECTIONS.

- **Skill:** arctan-of-linear style
