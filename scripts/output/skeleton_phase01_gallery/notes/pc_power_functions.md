# Notes — `pc_power_functions` (`Power functions`)
...
- **Skill:** Student graph or evaluate for power functions.
- **D=0:** evaluate/graph power f(x)=x^n; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Evaluate }f(x)=x^{5}\text{ at }x=6.$` | `$7776$` | evaluate/rewrite |
| 8 | 101 | `$\text{Evaluate }f(x)=x^{6}\text{ at }x=4.$` | `$4096$` | evaluate/rewrite |
| 16 | 101 | `$\text{Evaluate }f(x)=x^{4}\text{ at }x=2.$` | `$16$` | evaluate/rewrite |
| 22 | 101 | `$\text{Evaluate }f(x)=x^{5}\text{ at }x=3.$` | `$243$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §3.3 | https://openstax.org/books/precalculus-2e/pages/3-3-power-functions-and-polynomial-functions | evaluate/graph power f(x)=x^n |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/3-3-power-functions-and-polynomial-functions.md`
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_power_functions`.
