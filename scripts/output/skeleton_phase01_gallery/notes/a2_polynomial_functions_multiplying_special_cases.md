# Notes — `a2_polynomial_functions_multiplying_special_cases`

> **LOW_VARIETY** — One template across seeds at D=0.

- **Display name:** Multiplying special cases
- **Category:** Algebra 2 — Polynomial Functions
- **Generator:** `polynomial_multiply_special`
- **Already on skeleton?** yes (`multiply_special`)
...
- **Skill:** Practice multiplying special cases (catalog: polynomial_multiply_special).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | $\text{Multiply: } \left(x + 1\right)\left(x - 1\right)$ | $x^{2} - 1$ | — |
| 0 | 207 | $\text{Multiply: } \left(x + 3\right)\left(x - 3\right)$ | $x^{2} - 9$ | — |
| 8 | 101 | $\text{Multiply: } \left(3x + 3\right)\left(3x - 3\right)$ | $9x^{2} - 9$ | — |
| 8 | 207 | $\text{Multiply: } \left(2x + 1\right)\left(2x - 1\right)$ | $4x^{2} - 1$ | — |
| 16 | 101 | $\text{Multiply: } \left(2x + 4\right)^{2}$ | $4x^{2} + 16x + 16$ | — |
| 16 | 207 | $\text{Multiply: } \left(2x + 1\right)\left(2x - 1\right)$ | $4x^{2} - 1$ | — |
| 22 | 101 | $\text{Multiply: } \left(2x + 4\right)^{2}$ | $4x^{2} + 16x + 16$ | — |
| 22 | 207 | $\text{Multiply: } \left(2x + 1\right)\left(2x - 1\right)$ | $4x^{2} - 1$ | — |

Opt-out flag used: `{"use_factor_poly": true}`

## A1 alias comparison

- **A1 catalog twin:** `polynomial_multiply_special` (same generator `polynomial_multiply_special` unless noted).
...
- **Reuse:** Existing skeleton slug `multiply_special` / shared `polynomial_multiply_special` family.
- **New:** only if no honest match — leave leaf and document skip.
- **Not this pass:** notes only; no generator implementation.

_Catalog generator `polynomial_multiply_special`; equations/WP agent owns solve/WP siblings._
