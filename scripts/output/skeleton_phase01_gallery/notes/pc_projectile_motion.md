# Notes — `pc_projectile_motion` (`Projectile motion`)

- **Course:** Precalculus
- **Category:** Precalculus — Parametric Equations
...
- **Skill:** Student solve the problem for projectile motion.
- **D=0:** height/time from quadratic motion; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$h(t)=-16t^2+224t.\quad\text{Find the time of maximum height.}$` | `$7$` | evaluate/rewrite |
| 8 | 101 | `$h(t)=-16t^2+192t.\quad\text{Find the time of maximum height.}$` | `$6$` | evaluate/rewrite |
| 16 | 101 | `$h(t)=-16t^2+96t.\quad\text{Find the time of maximum height.}$` | `$3$` | evaluate/rewrite |
| 22 | 101 | `$h(t)=-16t^2+256t.\quad\text{Find the time of maximum height.}$` | `$8$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §8.6 | https://openstax.org/books/precalculus-2e/pages/8-6-parametric-equations | height/time from quadratic motion |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/8-6-parametric-equations.md`
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_projectile_motion`.
