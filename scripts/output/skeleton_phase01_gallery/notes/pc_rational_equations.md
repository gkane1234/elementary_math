# Notes — `pc_rational_equations` (`Rational equations`)
...
- **Skill:** Student solve for x for rational equations.
- **D=0:** solve rational eqn (LCD); simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Solve: } \frac{1}{2} = \frac{x}{3}$` | `$x = \frac{3}{2}$` | solve |
| 8 | 101 | `$\text{Solve: } \frac{5}{3} = \frac{z}{6}$` | `$z = 10$` | solve |
| 16 | 101 | `$\text{Solve: } \frac{4}{2v + 13} = \frac{1}{2v - 5}$` | `$v = \frac{11}{2},\; v \neq -\frac{13}{2}, \frac{5}{2}$` | solve |
| 22 | 101 | `$\text{Solve: } \frac{\tau}{\tau - 4} = \frac{4}{\tau - 4}$` | `$\emptyset$` | solve |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §3.7 | https://openstax.org/books/precalculus-2e/pages/3-7-rational-functions | solve rational eqn (LCD) |
...
- **Reuse:** Reuse rational skeleton / eq_cancel family.
