# Notes — `pc_graphing_logarithmic_functions` (`Graphing logarithmic functions`)
...
- **Skill:** Student graph the following functions for graphing logarithmic functions.
- **D=0:** graph log with vertical shift; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$y = \log_{2}\left(x\right)$` | `$y = \log_{2}\left(x\right)$` | evaluate/rewrite |
| 8 | 101 | `$y = 2\log_{2}\left(x + 1\right) - 1$` | `$y = 2\log_{2}\left(x + 1\right) - 1$` | evaluate/rewrite |
| 16 | 101 | `$y = \log_{2}\left(x + 1\right) - 1$` | `$y = \log_{2}\left(x + 1\right) - 1$` | evaluate/rewrite |
| 22 | 101 | `$y = \log_{2}\left(x + 1\right) - 1$` | `$y = \log_{2}\left(x + 1\right) - 1$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §4.4 | https://openstax.org/books/precalculus-2e/pages/4-4-graphs-of-logarithmic-functions | graph log with vertical shift |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/4-4-graphs-of-logarithmic-functions.md`
...
- **Reuse:** Graph framework for log shifts.
