# Notes — `pc_graphing_trig_functions` (`Graphing trig functions`)
...
- **Skill:** Student graph the following functions for graphing trig functions.
- **D=0:** amplitude/period/phase of sin/cos; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Graph } y=\sin x\text{ over one period.}$` | `$\text{amplitude }1,\ \text{period }2\pi,\ \text{midline }y=0$` | graph task |
| 8 | 101 | `$\text{Graph } y=2\cos x\text{ over one period.}$` | `$\text{amplitude }2,\ \text{period }2\pi,\ \text{midline }y=0$` | graph task |
| 16 | 101 | `$\text{Graph } y=-4\sin\left(3x-\frac{\pi}{2}\right)\text{ over one period.}$` | `$\text{amplitude }4,\ \text{period }\frac{2\pi}{3},\ \text{midline }y=0,\ \text{reflection over the x-axis}$` | graph task |
| 22 | 101 | `$\text{Graph } y=-5\sin\left(2x-\frac{\pi}{2}\right)\text{ over one period.}$` | `$\text{amplitude }5,\ \text{period }\pi,\ \text{midline }y=0,\ \text{reflection over the x-axis}$` | graph task |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §6.1 | https://openstax.org/books/precalculus-2e/pages/6-1-graphs-of-the-sine-and-cosine-functions | amplitude/period/phase of sin/cos |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/6-1-graphs-of-the-sine-and-cosine-functions.md`
...
- **Reuse:** Graph framework (amplitude/period).
