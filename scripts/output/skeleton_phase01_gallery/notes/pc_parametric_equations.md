# Notes — `pc_parametric_equations` (`Parametric equations`)
...
- **Skill:** Student solve or graph for parametric equations.
- **D=0:** eliminate parameter / point at t; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$x=t^2,\ y=2t+1.\quad\text{Find }(x,y)\text{ when }t=4.$` | `$(16, 9)$` | evaluate/rewrite |
| 8 | 101 | `$x=t^2,\ y=2t+1.\quad\text{Find }(x,y)\text{ when }t=2.$` | `$(4, 5)$` | evaluate/rewrite |
| 16 | 101 | `$x=t^2,\ y=2t+1.\quad\text{Find }(x,y)\text{ when }t=3.$` | `$(9, 7)$` | evaluate/rewrite |
| 22 | 101 | `$x=t^2,\ y=2t+1.\quad\text{Find }(x,y)\text{ when }t=3.$` | `$(9, 7)$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §8.6 | https://openstax.org/books/precalculus-2e/pages/8-6-parametric-equations | eliminate parameter / point at t |
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_parametric_equations`.
