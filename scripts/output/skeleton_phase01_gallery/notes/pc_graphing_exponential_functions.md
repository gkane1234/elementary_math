# Notes — `pc_graphing_exponential_functions` (`Graphing exponential functions`)
...
- **Skill:** Student graph the following functions for graphing exponential functions.
- **D=0:** graph y=a·b^x with shifts; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$y = 2^{x}$` | `$y = 2^{x}$` | evaluate/rewrite |
| 8 | 101 | `$y = 3 \cdot \left(\frac{1}{4}\right)^{x} + 3$` | `$y = 3 \cdot \left(\frac{1}{4}\right)^{x} + 3$` | evaluate/rewrite |
| 16 | 101 | `$y = -3^{x + 1} - 1$` | `$y = -3^{x + 1} - 1$` | evaluate/rewrite |
| 22 | 101 | `$y = -3^{x + 1} - 1$` | `$y = -3^{x + 1} - 1$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §4.2 | https://openstax.org/books/precalculus-2e/pages/4-2-graphs-of-exponential-functions | graph y=a·b^x with shifts |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/4-2-graphs-of-exponential-functions.md`
...
- **Reuse:** Graph framework; exp parameters from settings.
