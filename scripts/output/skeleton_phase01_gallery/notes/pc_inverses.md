# Notes — `pc_inverses` (`Inverses`)

- **Course:** Precalculus
- **Category:** Precalculus — Functions
- **Generator:** `inverse_function_basic`
...
- **Skill:** Student find the inverse for inverses.
- **D=0:** find inverse of linear/rational; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Find the inverse of } f(x) = x - 1$` | `$f^{-1}(x) = x + 1$` | evaluate/rewrite |
| 8 | 101 | `$\text{Find the inverse of } f(x) = -2x + 7$` | `$f^{-1}(x) = \frac{1}{-2}(x + 3.5)$` | evaluate/rewrite |
| 16 | 101 | `$\text{Find the inverse of } f(x) = -2x + 2$` | `$f^{-1}(x) = \frac{1}{-2}(x + 1)$` | evaluate/rewrite |
| 22 | 101 | `$\text{Find the inverse of } f(x) = 4x - 8$` | `$f^{-1}(x) = \frac{1}{4}(x + 2)$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §1.7 | https://openstax.org/books/precalculus-2e/pages/1-7-inverse-functions | find inverse of linear/rational |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/1-7-inverse-functions.md`
...
- **Reuse:** Reuse inverse generator; match A1/A2 inverse shapes.
