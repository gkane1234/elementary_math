# Notes — `pc_simple_trig_equations` (`Simple trig equations`)
...
- **Skill:** Student solve for x for simple trig equations.
- **D=0:** sin/cos/tan = k on principal interval; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\sin(x) = -\frac{1}{2}$` | `$x = 210^\circ, 330^\circ$` | evaluate/rewrite |
| 8 | 101 | `$\sin(x) = -\frac{1}{2}$` | `$x = 210^\circ, 330^\circ$` | evaluate/rewrite |
| 16 | 101 | `$\sin(x) = -\frac{1}{2}$` | `$x = 210^\circ, 330^\circ$` | evaluate/rewrite |
| 22 | 101 | `$\sin(x) = -\frac{1}{2}$` | `$x = 210^\circ, 330^\circ$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §7.5 | https://openstax.org/books/precalculus-2e/pages/7-5-solving-trigonometric-equations | sin/cos/tan = k on principal interval |
...
- **Reuse:** Trig eqn skeleton + OpenStax `precalculus_trig_equations.json`.
...
- **Variety:** Old-path table shows one prompt shape across D bands sampled (seed=101) — `$\sin(x) = -\frac{1}{2}$`.
