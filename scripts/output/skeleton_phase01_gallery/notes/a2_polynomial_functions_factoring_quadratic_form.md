# Notes — `a2_polynomial_functions_factoring_quadratic_form`

> **LOW_VARIETY** — One template across seeds at D=0.

- **Display name:** Factoring quadratic form
- **Category:** Algebra 2 — Polynomial Functions
- **Generator:** `a2_polynomial_functions_factoring_quadratic_form`
- **Already on skeleton?** yes (`factor_quadratic_form`)
...
- **Skill:** Practice factoring quadratic form (catalog: a2_polynomial_functions_factoring_quadratic_form).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | $x^{4} + 7x^{2} + 12$ | $\left(x^{2} + 3\right)\left(x^{2} + 4\right)$ | — |
| 0 | 207 | $x^{4} + 3x^{2} + 2$ | $\left(x^{2} + 1\right)\left(x^{2} + 2\right)$ | — |
| 8 | 101 | $4x^{4} - 24x^{2} + 32$ | $4\left(x^{2} - 2\right)\left(x^{2} - 4\right)$ | — |
| 8 | 207 | $x^{4} - 10x^{2} + 16$ | $\left(x^{2} - 2\right)\left(x^{2} - 8\right)$ | — |
| 16 | 101 | $4\left(-6x^{2}\right) + 32 + 4x^{4}$ | $4\left(x^{2} - 2\right)\left(x^{2} - 4\right)$ | — |
| 16 | 207 | $18 + 3x^{4} - 2x^{4} - 9x^{2}$ | $\left(x^{2} - 6\right)\left(x^{2} - 3\right)$ | — |
| 22 | 101 | $-x^{4} - 24x^{2} + 28 + 2\left(x^{4} + 2\right) + 3x^{4}$ | $4\left(x^{2} - 2\right)\left(x^{2} - 4\right)$ | — |
| 22 | 207 | $2\left(x^{4} + 1\right) - x^{4} - 9x^{2} + 16$ | $\left(x^{2} - 6\right)\left(x^{2} - 3\right)$ | — |
...
- **Reuse:** Existing skeleton slug `factor_quadratic_form` / shared `a2_polynomial_functions_factoring_quadratic_form` family.
- **New:** only if no honest match — leave leaf and document skip.
- **Not this pass:** notes only; no generator implementation.

_Catalog generator `a2_polynomial_functions_factoring_quadratic_form`; equations/WP agent owns solve/WP siblings._
