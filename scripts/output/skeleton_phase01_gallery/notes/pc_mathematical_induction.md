# Notes — `pc_mathematical_induction` (`Mathematical induction`)
...
- **Skill:** Student prove by mathematical induction for mathematical induction.
- **D=0:** verify base case induction; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Verify the base case }n=2\text{ for }\sum_{k=1}^n k=\frac{n(n+1)}2.$` | `$1+2=3$` | evaluate/rewrite |
| 8 | 101 | `$\text{Verify the base case }n=2\text{ for }\sum_{k=1}^n k=\frac{n(n+1)}2.$` | `$1+2=3$` | evaluate/rewrite |
| 16 | 101 | `$\text{Verify the base case }n=1\text{ for }\sum_{k=1}^n k=\frac{n(n+1)}2.$` | `$1=1$` | evaluate/rewrite |
| 22 | 101 | `$\text{Verify the base case }n=2\text{ for }\sum_{k=1}^n k=\frac{n(n+1)}2.$` | `$1+2=3$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §11.3 | https://openstax.org/books/precalculus-2e/pages/11-3-proofs | verify base case induction |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/11-3-proofs.md`
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_mathematical_induction`.
