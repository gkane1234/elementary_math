# Notes — `pc_piecewise_functions` (`Piecewise functions`)
...
- **Skill:** Student evaluate or graph for piecewise functions.
- **D=0:** evaluate piecewise at a point; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$f(x)=\begin{cases}x+0,&x<-2\\0,&x\ge-2\end{cases}.\quad\text{Evaluate }f(2).$` | `$0$` | evaluate/rewrite |
| 8 | 101 | `$f(x)=\begin{cases}1x^2+3,&x<-2\\3,&x\ge-2\end{cases}.\quad\text{Evaluate }f(0).$` | `$3$` | evaluate/rewrite |
| 16 | 101 | `$f(x)=\begin{cases}x-6,&x<-1\\-6,&-1\le x<1\\-5x,&x\ge1\end{cases}.\quad\text{Evaluate }f(-2).$` | `$-8$` | evaluate/rewrite |
| 22 | 101 | `$f(x)=\begin{cases}x+8,&x<-1\\8,&-1\le x<1\\6x,&x\ge1\end{cases}.\quad\text{Evaluate }f(-3).$` | `$5$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §1.5 | https://openstax.org/books/precalculus-2e/pages/1-5-transformation-of-functions | evaluate piecewise at a point |
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_piecewise_functions`.
