# Notes — `u_sub_preset_du_over_u` (`calc_indef_int_logarithmic_rule_and_exponentials_with_substitution`)

Showcase gallery for a **named u-sub knob**, not a separate catalog leaf.
Forces `u_sub_form_preset=du_over_u` and `u_sub_construction=catalog` via
`extra_settings` in `gen_examples.py` SECTIONS.

- **Skill:** integrals of the form g'/g
