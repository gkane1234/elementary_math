# Notes — `pc_ellipses_writing_equations` (`Ellipses, writing equations`)
...
- **Skill:** Student write the equation of the ellipse for ellipses, writing equations.
- **D=0:** write ellipse from vertices; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Write the equation of an ellipse centered at the origin with }a=5\text{ and }b=2.$` | `$\frac{x^2}{25}+\frac{y^2}{4}=1$` | evaluate/rewrite |
| 8 | 101 | `$\text{Write the equation of an ellipse centered at }(1,-1)\text{ with }a=7\text{ and }b=3.$` | `$\frac{(x-(1))^2}{49}+\frac{(y-(-1))^2}{9}=1$` | evaluate/rewrite |
| 16 | 101 | `$\text{Write the equation of an ellipse centered at }(-2,3)\text{ with }a=7\text{ and }b=8.$` | `$\frac{(x-(-2))^2}{49}+\frac{(y-(3))^2}{64}=1$` | evaluate/rewrite |
| 22 | 101 | `$\text{Write the equation of an ellipse centered at }(-1,1)\text{ with }a=6\text{ and }b=10.$` | `$\frac{(x-(-1))^2}{36}+\frac{(y-(1))^2}{100}=1$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §10.3 | https://openstax.org/books/precalculus-2e/pages/10-3-the-ellipse | write ellipse from vertices |
