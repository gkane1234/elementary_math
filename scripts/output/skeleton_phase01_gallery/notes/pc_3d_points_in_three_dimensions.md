# Notes — `pc_3d_points_in_three_dimensions` (`Points in three dimensions`)
...
- **Skill:** Student find the distance or coordinates for points in three dimensions.
- **D=0:** distance in 3D; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Find the distance from }(0,0,0)\text{ to }(4,2,4).$` | `$6$` | evaluate/rewrite |
| 8 | 101 | `$\text{Find the distance from }(0,0,0)\text{ to }(2,6,5).$` | `$\sqrt{65}$` | evaluate/rewrite |
| 16 | 101 | `$\text{Find the distance from }(0,0,0)\text{ to }(4,4,7).$` | `$9$` | evaluate/rewrite |
| 22 | 101 | `$\text{Find the distance from }(0,0,0)\text{ to }(2,3,6).$` | `$7$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §9.8 | https://openstax.org/books/precalculus-2e/pages/9-8-vectors | distance in 3D |
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_3d_points_in_three_dimensions`.
