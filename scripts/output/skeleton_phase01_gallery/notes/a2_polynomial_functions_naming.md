# Notes — `a2_polynomial_functions_naming`

> **UNCLEAR** — Old path names by degree (quadratic/cubic) only; OpenStax also asks monomial/binomial/trinomial.

- **Display name:** Naming
- **Category:** Algebra 2 — Polynomial Functions
- **Generator:** `polynomial_naming`
- **Already on skeleton?** yes (`polynomial_naming`)
...
- **Skill:** Practice naming (catalog: polynomial_naming).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | $\text{Name the polynomial: } x^{2} + 2$ | $\text{quadratic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 0 | 207 | $\text{Name the polynomial: } 2x^{2} + 3$ | $\text{quadratic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 8 | 101 | $\text{Name the polynomial: } 2x^{3} + x^{2} + 2$ | $\text{cubic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 8 | 207 | $\text{Name the polynomial: } x^{3} + 2x^{2} + 2$ | $\text{cubic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 16 | 101 | $\text{Name the polynomial: } 3x^{4} + 2x^{3} + x + 4$ | $\text{quartic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 16 | 207 | $\text{Name the polynomial: } x^{4} + 3x^{3} + x - 1$ | $\text{quartic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 22 | 101 | $\text{Name the polynomial: } 3x^{4} + 2x^{3} + x + 4$ | $\text{quartic}$ | pattern=PolyNaming, form=poly_degree_classify |
| 22 | 207 | $\text{Name the polynomial: } x^{4} + 3x^{3} + x - 1$ | $\text{quartic}$ | pattern=PolyNaming, form=poly_degree_classify |

Opt-out flag used: `(none — live default is old path)`

## A1 alias comparison

- **A1 catalog twin:** `polynomial_naming` (same generator `polynomial_naming` unless noted).
...
Old path names by degree (quadratic/cubic) only; OpenStax also asks monomial/binomial/trinomial.
Flags: `UNCLEAR`.
...
- **Reuse:** Existing skeleton slug `polynomial_naming` / shared `polynomial_naming` family.
- **New:** only if no honest match — leave leaf and document skip.
- **Not this pass:** notes only; no generator implementation.

_Catalog generator `polynomial_naming`; equations/WP agent owns solve/WP siblings._
