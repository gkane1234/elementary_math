...
- **D=8 seed=101:** $\text{Factor: } 3x + 6$ → $3\left(x + 2\right)$ — form_id=gcf_monomial
- **D=8 seed=207:** $\text{Factor: } 2 + 2x$ → $2\left(x + 1\right)$ — form_id=gcf_monomial
- **D=16 seed=101:** $\text{Factor: } 3x + 6$ → $3\left(x + 2\right)$ — form_id=gcf_monomial
- **D=16 seed=207:** $\text{Factor: } 2 + 2x$ → $2\left(x + 1\right)$ — form_id=gcf_monomial
- **D=22 seed=101:** $\text{Factor: } 3x + 6$ → $3\left(x + 2\right)$ — form_id=gcf_monomial
- **D=22 seed=207:** $\text{Factor: } 2 + 2x$ → $2\left(x + 1\right)$ — form_id=gcf_monomial
...
**UNCLEAR** — catalog alias `g6_factor_gcf` is **not a live type_id** (`Unknown question type`); live leaf is `polynomial_factoring_common_factor`. **LOW_VARIETY** — **default** skeleton repeats $3x+6$ / $2+2x$ at every D. Old opt-out *does* add $x$ in the GCF at D≥8 (e.g. $4x(2x-3)$). OpenStax §7.1 is still richer (two variables).

## Proposed engine (reuse vs new) — proposal only

- **Reuse:** FactorGcf / poly_skeleton (already wired). Do not implement here; D-ladder is thin.
