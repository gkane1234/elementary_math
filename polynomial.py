...
class Polynomial:
    ...
    def __str__(self,rounding=5):
        ...
        return a.__repr__()
    def toLatexString(self,rounding=5) :
        a=round(self,rounding)
    ...
    def __eq__(self, o):
        if type(o)==Polynomial:
            return o.coefList(reverse=True)==self.coefList(reverse=True)
        return False
    def __len__(self):
    ...
    def coefList(self,reverse=False):
    ...
    def PFD(self,others) : 
        ...
        diffs=[]
        for i in prods:
            termCoef=i.coefList(reverse=True)
        ...
        thisCoefs=self.coefList(reverse=True)+([0]*(degree-len(self.coefList(reverse=True))))
    ...
    def displayFactors(others):
        ...
        return '('+str(others)+')'
    @staticmethod
    def createPolynomialWithIntegerFactorsRanges(leadingCoefficientRange,coefficientRange,factors,returnFactors=False):
        ...
        for i in range(1,factors+1):
            g=Polynomial([random.randint(minLead, maxLead),(2*random.randint(0, 1)-1)*random.randint(minCoef, maxCoef)])
    ...
    def checker(trials=100000,minDeg=3,maxDeg=30,minCoef=-100,maxCoef=100,tolerance=4,integer=True,output=False)  :
        ...
        for i in range(trials) :
            ...
            if output or (not (round(x,tolerance)==round(p,tolerance))) :
                print("original:"+str(p.coefList()),"\n attempt:"+str(x.coefList()),"\n Factors: ")
                print('(',end='')
                print(*a,sep=')(',end='')
                print(')')
                count+=1
...
# Generate two random binomials with random leading coefficients and constants
binomial1 = Polynomial((random.randint(1, 100000), random.randint(-100000, 100000)))
binomial2 = Polynomial((random.randint(1, 100000), random.randint(-100000, 100000)))
...
#b = Polynomial((1,3))*Polynomial((3,-4))

#c = Polynomial((1,0,-2))*Polynomial((1,-4))



#print(Polynomial.createRationalAdditionProblemWithOneCancellation((1,3), (1,3)))
...
#print(Polynomial((2,11))*Polynomial((1,2))+Polynomial((1,-19))*Polynomial((2,1)))
