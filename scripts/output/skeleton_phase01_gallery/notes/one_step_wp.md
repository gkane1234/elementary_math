...
- **D=8 seed=101:** $\text{Will had some money, received \$3, and now has \$8. How much did Will start with?}$ → $\5$ — frame_id=money_received, steps=one
- **D=8 seed=207:** $\text{Tickets cost \$4 each. Kevin spends \$44. How many tickets did Kevin buy?}$ → $11\text{ tickets}$ — frame_id=count_tickets, steps=one
- **D=16 seed=101:** $\text{Kim had some money, received \$7, and now has \$13. How much did Kim start with?}$ → $\6$ — frame_id=money_received, steps=one
- **D=16 seed=207:** $7\text{ less than a number is }13\text{. What is the number?}$ → $20$ — frame_id=number_one_step, steps=one
- **D=22 seed=101:** $\text{Kim divided some cups into 5 equal groups with 7 in each group. How many cups were there?}$ → $35\text{ cups}$ — frame_id=count_groups, steps=one
- **D=22 seed=207:** $\text{Lena shared \$140 equally among 7 friends. How much did each friend receive?}$ → $\20$ — frame_id=money_shared, steps=one
