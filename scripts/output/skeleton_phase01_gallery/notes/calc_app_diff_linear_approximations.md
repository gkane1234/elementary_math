...
- **Skill:** Find the linearization \(L(x)\) of \(f\) at a point, or use \(L\) to estimate a nearby value.
- **D=0:** Easy leftover mix: \(f(x)=x^{2}\) or \(f(x)=\sqrt{x}\) at a nice \(a\) (old easy; OpenStax Ex. 4.5 formula half).
- **Mid D (≈8):** Easy leftover still allowed, plus estimate-\(x^{2}\), estimate-\(\sqrt{x}\) (Ex. 4.5), \(1/x\), and \(e^{x}\) at \(0\).
- **High D (≈16):** Lock out \(x^{2}\) leftovers (formula and estimate). \(\sqrt{x}\) leftover (formula and estimate) plus \(1/x\) and \(e^{x}\).
...
Leftover lockout on the same four family builders, plus Ex. 4.5 estimate-\(\sqrt{x}\) on the existing \(\sqrt{x}\) linearization + estimate stem (`sqrt_estimate`). D=0 stays the old \(\sqrt{x}\) / \(x^{2}\) mix (formula only). Gallery seeds 101/207/313 can collide on one form at a band; rotation is across seeds.
...
| 8 | 101 | $\text{Use the linear approximation of }f(x)=\sqrt{x}\text{ at }x=1\text{ to estimate }\sqrt{\frac{3}{2}}.$ | $\frac{5}{4}$ | `sqrt_estimate` (Ex. 4.5) |
...
| 16 | 7 | $\text{Use the linear approximation of }f(x)=\sqrt{x}\text{ at }x=1\text{ to estimate }\sqrt{\frac{6}{5}}.$ | $\frac{11}{10}$ | `sqrt_estimate` leftover |
...
40-seed counts **after**: D=0 sqrt 21 / quad 19; D=8 leftover easy + `quad_estimate` 5 / `sqrt_estimate` 10 / reciprocal 5 / exp 6; D=16 sqrt 11 / `sqrt_estimate` 8 / reciprocal 14 / exp 7 (no quad); D=22 exp 21 / reciprocal 19 only.
...
Not a WP. Algebra/technique shapes follow old path (\(x^{2}\), \(\sqrt{x}\) at \(\{1,4,9\}\), \(1/x\) at \(a\in\{2,\ldots,5\}\), \(e^{x}\) at \(0\), estimate \(h\in\{1/10,1/5,1/2\}\)). Estimate-\(\sqrt{x}\) reuses the existing \(\sqrt{x}\) \(L(x)\) and the leftover estimate stem — not a new core. Do not invent sin / cube-root / \((1+x)^{n}\) cores this pass.

## Limitations

- **Status:** shipped — leftover lockout of D=0 \(x^{2}\) (formula + estimate) plus Ex. 4.5 estimate-\(\sqrt{x}\) on the existing \(\sqrt{x}\) linearization. Remaining `LIMITATIONS`: no Ex. 4.6 \(\sin x\) at \(\pi/3\) / degree conversion; no Checkpoint 4.5 cube root; no Ex. 4.7 \((1+x)^{n}\) at \(0\); D=16 can still emit \(\sqrt{x}\) leftover formula/estimate (intentional); D=22 \(e^{x}\) is frozen at \(a=0\); no diagram of \(y\) vs \(L(x)\).

## Proposed engine (reuse vs new)

- **Reuse:** existing family builders (now in `calc_app_diff.py`; estimate is `quad_estimate` / `sqrt_estimate` for `select_form_id` / `live_quality_form_weights`). Depth = real structure (lock out \(x^{2}\); Ex. 4.5 estimate on leftover \(\sqrt{x}\); mix leftover at D=8 / D=16; D=22 reciprocal/exp only) — not padded `difficulty_costs`.
