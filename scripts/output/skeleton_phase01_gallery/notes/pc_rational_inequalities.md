# Notes — `pc_rational_inequalities` (`Rational inequalities`)
...
- **Skill:** Student solve the inequality for rational inequalities.
- **D=0:** critical points on rational inequality; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\frac{x-6}{x+3}>0.$` | `$x<-3\text{ or }x>6$` | evaluate/rewrite |
| 8 | 101 | `$\frac{x-4}{x+5}>0.$` | `$x<-5\text{ or }x>4$` | evaluate/rewrite |
| 16 | 101 | `$\frac{x-3}{x+4}>0.$` | `$x<-4\text{ or }x>3$` | evaluate/rewrite |
| 22 | 101 | `$\frac{x-5}{x+2}>0.$` | `$x<-2\text{ or }x>5$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §3.7 | https://openstax.org/books/precalculus-2e/pages/3-7-rational-functions | critical points on rational inequality |
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_rational_inequalities`.
