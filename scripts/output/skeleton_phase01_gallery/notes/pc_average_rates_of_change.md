# Notes — `pc_average_rates_of_change` (`Average rates of change`)
...
- **Skill:** Student find the average rate of change for average rates of change.
- **D=0:** average ROC on interval for polynomial; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{Find the average rate of change of }f(x)=x^2\text{ on }[1,3].$` | `$4$` | evaluate/rewrite |
| 8 | 101 | `$\text{Find the average rate of change of }f(x)=x^2\text{ on }[-1,5].$` | `$4$` | evaluate/rewrite |
| 16 | 101 | `$\text{Find the average rate of change of }f(x)=x^2\text{ on }[1,4].$` | `$5$` | evaluate/rewrite |
| 22 | 101 | `$\text{Find the average rate of change of }f(x)=x^2\text{ on }[2,5].$` | `$7$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §1.3 | https://openstax.org/books/precalculus-2e/pages/1-3-rates-of-change-and-behavior-of-graphs | average ROC on interval for polynomial |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/1-3-rates-of-change-and-behavior-of-graphs.md`
...
- **Reuse:** Keep live `precalc_foundations` until dedicated skeleton; split topic branches per OpenStax section. Topic key `pc_average_rates_of_change`.
