# Notes — `pc_radians_and_degrees` (`Radians and degrees`)
...
- **Skill:** Student convert between radians and degrees for radians and degrees.
- **D=0:** convert + unit-circle special angles; simplest numeric/instance (per OpenStax § easy exercises).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | `$\text{An angle on the unit circle has } \sin = \frac{\sqrt{2}}{2}. \text{Which standard angle (in degrees) has this value?}$` | `$135$` | evaluate/rewrite |
| 8 | 101 | `$\text{An angle on the unit circle has } \cos = \frac{\sqrt{2}}{2}. \text{Which standard angle (in degrees) has this value?}$` | `$45$` | evaluate/rewrite |
| 16 | 101 | `$\text{Find } \cos\left(0^\circ\right) \text{ on the unit circle.}$` | `$1$` | evaluate/rewrite |
| 22 | 101 | `$\text{Find } \cos\left(60^\circ\right) \text{ on the unit circle.}$` | `$\frac{1}{2}$` | evaluate/rewrite |
...
|------|-----|-------------------------------------------|
| OpenStax Precalculus 2e §5.1 | https://openstax.org/books/precalculus-2e/pages/5-1-angles | convert + unit-circle special angles |

Local HTML (if mined): `textbooks/openstax/html/precalculus-2e/…` or `scripts/output/example_mining/precalculus-2e/stage1/5-1-angles.md`
...
- **Reuse:** Unit-circle angle skeleton (exact values).
