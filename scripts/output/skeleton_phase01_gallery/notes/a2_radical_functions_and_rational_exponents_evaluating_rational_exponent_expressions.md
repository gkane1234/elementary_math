# Notes — `a2_radical_functions_and_rational_exponents_evaluating_rational_exponent_expressions`

> **LOW_VARIETY** — One template across seeds at D=0.

- **Display name:** Evaluating rational exponent expressions
...
- **Skill:** Practice evaluating rational exponent expressions (catalog: properties_of_exponents).
...
|---|------|--------------|--------------|-------------|
| 0 | 101 | $a^{2} \cdot a^{2}$ | $a^{4}$ | — |
| 0 | 207 | $a^{1} \cdot a^{1}$ | $a^{2}$ | — |
| 8 | 101 | $\left(x^{5}\right)^{3}$ | $x^{15}$ | — |
| 8 | 207 | $\left(a^{5}\right)^{2}$ | $a^{10}$ | — |
| 16 | 101 | $\frac{a^{7}}{a^{8}}$ | $a^{-1}$ | — |
| 16 | 207 | $\frac{a^{4}}{a^{1}}$ | $a^{3}$ | — |
| 22 | 101 | $x^{9} - x^{6}$ | $x^{9} - x^{6}$ | — |
| 22 | 207 | $a^{11} + a^{10}$ | $a^{11} + a^{10}$ | — |

Opt-out flag used: `(none — live default is old path)`

## A1 alias comparison

- **A1 catalog twin:** `properties_of_exponents` (same generator `properties_of_exponents` unless noted).
...
- **Reuse:** Share A1 `properties_of_exponents` generator; wire A2 catalog id when skeleton lands.
- **New:** only if no honest match — leave leaf and document skip.
- **Not this pass:** notes only; no generator implementation.

_Catalog generator `properties_of_exponents`; equations/WP agent owns solve/WP siblings._
