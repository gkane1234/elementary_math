...
Old path is **bike-only** at every seed. Default rotates car/bus/bike/walk/train (EA §2.6); D=0 asks for distance, later D asks time or rate.

## Proposed engine (reuse vs new) — proposal only

- **Reuse:** SolveLinear one-step + `wp_packaging` write_rate frames (wired; vehicles rotate).
